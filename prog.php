...
<body>
	...
	<div class="middle">
		<?php if(isset($_GET['date'])){
				?><form action="prog.php?date=<?php echo $_GET['date']; ?>&id_doctor=<?php echo $_GET['id_doctor']; ?>&id_depart=<?php echo $_GET['id_depart']; ?>" method="post" enctype="multipart/form-data"><?php 
		} else {
			?><form action="prog.php?date=<?php echo $_GET['ym']; ?>&id_doctor=<?php echo $_GET['id_doctor']; ?>&id_depart=<?php echo $_GET['id_depart']; ?>" method="post" enctype="multipart/form-data"><?php 
		} ?>
			...
			<div class="container">
				...
				<div style="width: 100%; overflow: auto;">
					<div style="float: left; width: 100px; text-align: center;">
						<a href="?ym=<?php echo $prev; ?>&id_doctor=<?php echo $_GET['id_doctor']; ?>&id_depart=<?php echo $_GET['id_depart']; ?>" class="link_prev">PREV</a>
					</div>
					<div style="float: left; width: 500px; text-align: center;">
						<?php echo $html_title; ?>
					</div>
					<div style="float: left; width: 100px; text-align: center;">
						<a href="?ym=<?php echo $next; ?>&id_doctor=<?php echo $_GET['id_doctor']; ?>&id_depart=<?php echo $_GET['id_depart']; ?>" class="link_next">NEXT</a>
					</div>
				</div>
				<div style="width: 100%; overflow: auto;">
					...
					<div style="margin: auto; text-align: center;"><strong>Select hour</strong>
						<select name="ora" id="ora"><option></option>
							<?php foreach($list_ora as $item){
								if(isset($_GET['date'])){
									$sel_pro = mysqli_query($conn, "select * from programari where id_depart = '".$_GET['id_depart']."' and id_doctor = '".$_GET['id_doctor']."' and data = '".$_GET['date']."' and ora = '".$item."'") or die (mysqli_error($conn));
									$nr = mysqli_num_rows($sel_pro);	
								} else { 
									$sel_pro = mysqli_query($conn, "select * from programari where id_depart = '".$_GET['id_depart']."' and id_doctor = '".$_GET['id_doctor']."' and data = '".$_GET['ym']."' and ora = '".$item."'") or die (mysqli_error($conn));
									$nr = mysqli_num_rows($sel_pro);
								}
